...
export default function NfcPassportWriter({ passportId }: { passportId: string }) {
  ...
  async function writeNfc() {
    ...
    setStatus("");
    try {
      const Reader = (window as Window & { NDEFReader: new () => { write: (message: { records: { recordType: "url"; data: string }[] }) => Promise<void> } }).NDEFReader;
      const reader = new Reader();
      const passportUrl = `${window.location.origin}/passport/${encodeURIComponent(passportId)}`;
      await reader.write({ records: [{ recordType: "url", data: passportUrl }] });
